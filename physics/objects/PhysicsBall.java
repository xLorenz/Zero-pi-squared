...
public class PhysicsBall extends PhysicsObject {
    ...
    public int[] getOccuppiedChunks(int chunkDim) {
        ...
        result[0] = (int) Math.floor((pos.x - radius) / chunkDim) - 1; // left; // minx
        result[1] = (int) Math.floor((pos.x + radius) / chunkDim) + 1; // right; // maxx
        result[2] = (int) Math.floor((pos.y - radius) / chunkDim) - 1; // down; // miny
        result[3] = (int) Math.floor((pos.y + radius) / chunkDim) + 1; // up; // maxy
    ...
    public Manifold collideWithRect(PhysicsRect rect) {
        ...
        if (m != null) {
            ...
            rect.onColisionWithCircle(this, m);
        }
        return m;
    }
